    ...
    >
      ...
      <div>
        ...
        @if (showOnboardingPrompt() && !dismissedOnboarding()) {
          <div
            class="bg-gradient-to-r from-primary-50 to-sage-50 dark:from-primary-900/20 dark:to-sage-900/20 border-b border-primary-100 dark:border-primary-800"
          >
            <div
              class="px-4 sm:px-8 py-4 flex flex-wrap items-center justify-between gap-4"
            >
              <div class="flex items-center gap-4">
                ...
                <div>
                  <p class="text-sm font-bold text-gray-900 dark:text-white">
                    {{ "DASHBOARD.COMPLETE_PROFILE_TITLE" | translate }}
                  </p>
                  <p class="text-xs text-gray-600 dark:text-gray-400">
                    {{ "DASHBOARD.COMPLETE_PROFILE_DESC" | translate }}
                  </p>
                </div>
              </div>
              <div class="flex items-center gap-2">
                <a
                  ...
                  class="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-primary-500 text-white font-semibold text-sm shadow-lg shadow-primary-500/25 hover:bg-primary-600 transition-all"
                >
                  {{ "DASHBOARD.START_ONBOARDING" | translate }} →
                </a>
                <button
                  (click)="dismissOnboarding()"
                  class="p-2 rounded-lg hover:bg-primary-100 dark:hover:bg-primary-800/50 transition-colors"
                >
                  <svg
        ...
        @if (
          auth.user() && !auth.isEmailVerified() && !dismissedEmailBanner()
        ) {
          <div
            class="bg-amber-50 dark:bg-amber-900/20 border-b border-amber-100 dark:border-amber-800"
          >
            <div
              class="px-4 sm:px-8 py-3 flex flex-wrap items-center justify-between gap-4"
            >
              <div class="flex items-center gap-3">
                <span
              ...
              <div class="flex items-center gap-2">
                <button
                  ...
                  class="text-sm font-bold text-amber-700 dark:text-amber-300 hover:underline"
                >
                  {{ "AUTH.RESEND_VERIFICATION" | translate }}
                </button>
                <button
                  (click)="dismissEmailBanner()"
                  class="p-2 rounded-lg hover:bg-amber-100 dark:hover:bg-amber-800/50 transition-colors"
                >
                  <svg
        ...
        <main class="px-4 sm:px-8 py-8 pb-24 space-y-10">
          ...
          >
            ...
            <div class="relative group hidden md:block">
              <div
                class="absolute top-full right-0 mt-3 bg-white dark:bg-gray-800 px-4 py-2.5 rounded-2xl shadow-xl border border-gray-100 dark:border-gray-700 animate-bounce z-10 w-64"
              >
                <p class="text-sm font-bold text-primary-500 italic">
                  "{{ mascotMessage() | translate }}"
                </p>
                <div
                  class="absolute -top-2 right-8 w-4 h-4 bg-white dark:bg-gray-800 border-b border-r border-gray-100 dark:border-gray-700 rotate-45"
                ></div>
              </div>
              <div
                class="w-32 h-32 bg-primary-50 dark:bg-primary-900/30 rounded-3xl flex items-center justify-center p-4 shadow-2xl shadow-primary-500/10 group-hover:scale-105 transition-transform cursor-pointer"
              >
          ...
          <section class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div
              #metricCard
              class="metric-card glass-panel p-8 rounded-3xl space-y-6"
            >
              <div class="flex justify-between items-center">
                <span
                  class="text-xs font-bold uppercase tracking-widest text-gray-500 dark:text-gray-400"
                  >{{ "METRICS.DAILY_ACTIVITY" | translate }}</span
                >
                <div
              ...
              <div class="flex items-center gap-6">
                ...
                <div>
                  ...
                  >
                    {{ "METRICS.STEPS_GOAL" | translate: { goal: "10k" } }}
                  </p>
                </div>
              </div>
            </div>
            ...
            >
              ...
              <div>
                <div class="flex items-baseline gap-2 mb-4">
                  <span
                    class="text-4xl font-bold text-gray-900 dark:text-white font-heading"
                    >{{ heartRate() }}</span
                  >
                  <span class="text-gray-500 dark:text-gray-400 font-medium">{{
                    "DASHBOARD.BPM" | translate
                  }}</span>
                </div>
                <div
                  class="h-16 w-full bg-gray-50 dark:bg-gray-800 rounded-2xl flex items-end gap-1.5 px-3 pb-3"
                >
            ...
            >
              ...
              <div>
                <p
                  class="text-4xl font-bold text-gray-900 dark:text-white font-heading mb-2"
                >
                  {{ sleepHours() }}h {{ sleepMinutes() }}m
                </p>
                <div class="flex items-center gap-2 mb-4">
                  <span
                    class="px-3 py-1 bg-green-100 dark:bg-green-900/30 text-green-600 dark:text-green-400 text-xs font-bold rounded-full"
                    >{{ "METRICS.EXCELLENT" | translate }}</span
                  >
                  <span
                    class="text-xs text-gray-500 dark:text-gray-400 font-medium"
                    >+12m vs {{ "METRICS.YESTERDAY" | translate }}</span
                  >
                </div>
                <div
                  class="w-full h-3 bg-gray-100 dark:bg-gray-800 rounded-full overflow-hidden"
                >
                  <div
          ...
          <section class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 glass-panel p-8 rounded-3xl">
              ...
              >
                <div>
                  <h3
                    class="text-2xl font-bold text-gray-900 dark:text-white font-heading"
                  >
                    {{ "METRICS.WEEKLY_TRENDS" | translate }}
                  </h3>
                ...
                <select
                  class="bg-gray-50 dark:bg-gray-800 border-none rounded-xl text-sm font-bold text-gray-600 dark:text-gray-300 px-4 py-2.5 cursor-pointer"
                >
              ...
              <!-- Bar Chart -->
              <div
                class="h-64 relative w-full flex items-end justify-between px-2 gap-4"
              >
                <div
                  class="absolute inset-x-0 bottom-0 h-32 bg-gradient-to-t from-primary-500/5 to-transparent rounded-3xl"
                ></div>
                  ...
                  <div class="flex-1 flex flex-col items-center gap-4">
                    ...
                    >
                      <div
                        ...
                        [class]="
                          day.isToday
                            ? 'bg-primary-500 shadow-[0_0_20px_rgba(185,5,56,0.3)]'
                            : 'bg-primary-400/60'
                        "
            ...
            <div class="glass-panel p-8 rounded-3xl">
              <div class="flex justify-between items-center mb-6">
                <h3
                  class="text-xl font-bold text-gray-900 dark:text-white font-heading"
                >
                  {{ "DASHBOARD.SCHEDULE" | translate }}
                </h3>
                <button
                  class="w-8 h-8 rounded-full bg-primary-50 dark:bg-primary-900/30 text-primary-500 flex items-center justify-center hover:bg-primary-100 dark:hover:bg-primary-900/50 transition-colors"
                >
                  <svg
                    class="w-4 h-4"
                    fill="none"
              ...
              <div class="space-y-4">
                @for (item of scheduleItems; track item.title; let i = $index) {
                  ...
                  >
                    ...
                    @if (item.status === "done") {
                      <span
                        class="self-start px-2.5 py-1 bg-green-100 dark:bg-green-900/30 text-green-600 dark:text-green-400 text-xs font-bold rounded-full uppercase"
                        >{{ "DASHBOARD.DONE" | translate }}</span
                      >
                    }
                  </div>
                }
          ...
          <section>
            <div class="flex justify-between items-center mb-6">
              <h3
                class="text-2xl font-bold text-gray-900 dark:text-white font-heading"
              >
                {{ "DASHBOARD.ACTIVITY_LOG" | translate }}
              </h3>
              <a
                routerLink="/journal"
                class="text-sm font-bold text-primary-500 hover:text-primary-600 transition-colors"
              >
            ...
            <div
              class="flex gap-4 overflow-x-auto pb-4 -mx-4 px-4 sm:-mx-8 sm:px-8 scrollbar-hide"
            >
...
export class DashboardComponent implements OnInit, AfterViewInit {
  ...
  @ViewChildren("scheduleItem") scheduleItems_els!: QueryList<
  ...
  mascotMessage = computed(() => {
    const msg =
      this.mascotMessages[
        Math.floor(Math.random() * this.mascotMessages.length)
      ];
    return msg;
  });
  ...
  constructor() {
    ...
    this.dismissedOnboarding.set(
      localStorage.getItem("vibehealth_dismissed_onboarding") === "true",
    );
    this.dismissedEmailBanner.set(
      localStorage.getItem("vibehealth_dismissed_email_banner") === "true",
    );
  }
